import React from 'react';
import CardPostPreviewAdminActions from '../atoms/CardPostPreviewAdminActions';
import CardPostPreviewUserActions from '../atoms/CardPostPreviewUserActions';
import Spinner from '../atoms/Spinner';

export default class PostPreview extends React.Component {
  renderActions() {
    if (this.props.isAdmin) {
      return (
        <CardPostPreviewAdminActions
          postId={this.props.post.id}
          history={this.props.history}
        />
      );
    }
    return <CardPostPreviewUserActions postId={this.props.post.id} />;
  }

  render() {
    if (!this.props.post) return <Spinner />;
    return (
      <div>
        <div
          className="card"
          style={{ width: '17.8rem', height: '98%', marginTop: '15px' }}>
          <img
            className="card-img-top"
            style={{ height: '12rem' }}
            src={this.getImage(this.props.post)}
            alt="Preview from post"
          />
          <div className="card-body">
            <h5 className="card-title">{this.props.post.title}</h5>
            <p className="card-text">{this.props.post.resume}</p>
            {this.renderActions()}
          </div>
        </div>
      </div>
    );
  }

  getImage(post) {
    if (post.previewImage) {
      return post.previewImage;
    }
    return 'data:image/png;base64,';
  }
}
